"""FastAPI service for the original live-game explorer.

Runs separately from SC2 and communicates through GameBridge queues. Endpoints
provide game status, observations, logs, action/fallback configuration, execution
control, process startup/shutdown, an SSE event stream, and available actions."""
    ...
    def load_etg(self, etg_file: str, data_dir: Optional[str] = None) -> bool:
        ...
        if data_dir:
            ...
            if len(parts) >= 2:
                ...
                self._dist_matrix = load_distances(map_id, data_id, etg_file)
                if self._dist_matrix is None:
                    logger.warning("No archived distances for %s", data_dir)
                else:
                    logger.info("Loaded distances for %s/%s", map_id, data_id)
