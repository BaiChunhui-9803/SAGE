"""Live SAGE agent built on SmartAgent's action execution methods.

Performs local state abstraction, graph planning, switch-aware plan following,
and optional gated action tuning. Replay mode consumes preset actions. Invalid
decisions use the configured fallback. Episode records are saved locally and
may also be published to the Web API in batches."""
...
class ETGGuidedAgent(SmartAgent):
    def __init__(
        self,
        bridge: GameBridge,
        ...
        super(ETGGuidedAgent, self).__init__()
        self.bridge = bridge
        self._fallback_action = fallback_action
        self._data_dir = data_dir
        self._etg_file = etg_file
        ...
        self._etg_sam: Optional[Dict[int, Dict]] = None
        ...
        if override_model_path and os.path.isfile(override_model_path):
            try:
                ...
                print(
                    f"[ETGGuidedAgent] failed to load override model: {_e}", flush=True
                )
        ...
        print(
            f"[ETGGuidedAgent] mode={self._mode}, state_id_map={len(self._state_id_map)}, "
            f"ETG={'loaded' if self.ETG else 'None'}, transitions={'loaded' if self.transitions else 'None'}, "
    ...
    def _serialize_bktree_node(node):
        ...
        return {
            ...
            "children": {
                str(dist): ETGGuidedAgent._serialize_bktree_node(child)
    ...
    def get_state_cluster(self, norm_state):
        ...
        self._last_bktree_match = {}
        return super(ETGGuidedAgent, self).get_state_cluster(norm_state)
    ...
    def _route_with_action_tuning(
        ...
        explore_sources = self._beam_params.get(
            ...
            ["fallback", "ft_plan", "etg_relaxed", "fuzzy_plan", "ood"],
        )
        validation_sources = self._beam_params.get(
            "tuning_validation_sources",
            ["ood", "fallback", "etg_relaxed", "fuzzy_plan", "diverge"],
        )
    ...
    def _path_summary_for_logging(self, path) -> Dict[str, Any]:
        try:
            from src.decision.etg_beam_search import _compute_path_composite
    ...
    def _get_plan_from_beam(
        ...
        if self.ETG is None or self.transitions is None:
            _dbg = f"[PLAN-DEBUG] early return: ETG={self.ETG is None}, transitions={self.transitions is None}"
    ...
    def _local_decide(
        ...
        if self._plan_idx < len(self._action_plan):
            ...
            if expected is not None and state_id == expected:
                ...
                self._plan_idx += 1
                return action, "etg_follow", self._last_plan_snap
        ...
        if self._plan_idx == 0:
            ...
            if masked and action in masked:
                ...
                self._write_plan_log(_msg)
            if self._plan_idx == 0:
                event_type = "diverge" if is_diverge else "etg_plan"
            else:
                event_type = "etg_follow"
    ...
    def _fallback_chain(self, state_id: int, trigger: str) -> Tuple[Optional[str], str]:
        ...
        if self.ETG is not None:
            relaxed = self.ETG.get_top_k_actions(
                state=state_id, k=1, min_visits=1, metric="quality_score"
            )
            if relaxed:
                action = relaxed[0][0]
                _msg = f"[PLAN] nid={state_id} trigger={trigger} -> etg_relaxed action={action}"
                self._write_plan_log(_msg)
                return action, "etg_relaxed"

        if self._dist_matrix is not None and self.ETG is not None:
            best_sid = None
            best_dist = float("inf")
            if getattr(self._dist_matrix, "is_sparse_distance_index", False):
                for sid, d in self._dist_matrix.get_neighbors(state_id, max_distance=0.3):
                    if sid in self.ETG.unique_states and d < best_dist:
                        best_dist = float(d)
                        best_sid = sid
            else:
                for sid in self.ETG.unique_states:
                    if sid == state_id:
                        continue
                    try:
            ...
            if best_sid is not None and best_dist < 0.3:
                fuzzy_actions = self.ETG.get_top_k_actions(
    ...
    def _flush_ep_batch(self, counters_snapshot=None):
        ...
        if self._local_result_dir:
            ...
            if (self._beam_params.get("pause_when_target_reached", False)
                    and target_int > 0 and self._local_completed >= target_int
                    and self.bridge is not None):
                # End a search trial at its requested sample count instead of
                # collecting extra episodes during the learner's polling delay.
                self.bridge.send_control("pause")
    ...
    def _load_etg_sam(self) -> None:
        if self._etg_sam is not None:
            return
        etg_file = self._etg_file or self._beam_params.get("etg_file")
        data_dir = self._data_dir or self._beam_params.get("data_dir")
        if not etg_file or not data_dir:
            return
        try:
            import pickle as _pickle
            from src import ROOT_DIR as _ROOT

            etg_path = _ROOT / "cache" / "experience_transition_graph" / etg_file
            ...
            if sam:
                self._etg_sam = sam
                print(f"[etg-SAM] loaded {len(sam)} states for ETG reward", flush=True)
        except Exception as e:
            print(f"[WARN] failed to load etg_sam: {e}", flush=True)
    ...
    def _etg_recalibrate(self) -> None:
        model = self._get_finetune_model()
        if model is None or self._etg_sam is None:
            return
        ...
        for sid, actions in model.q_table.items():
            if sid not in self._etg_sam:
                continue
            for ac, est in actions.items():
                if ac not in self._etg_sam[sid]:
                    continue
                etg_stats = self._etg_sam[sid][ac]
                try:
                    etg_v = float(etg_stats.avg_step_reward) + gamma * float(
                        etg_stats.avg_future_reward
                    )
    ...
    def step(self, obs, env):
        ...
        if self._mode != "replay":
            ...
            if c["total"] % self._log_interval == 0:
                exploit = c.get("etg_plan", 0) + c.get("etg_follow", 0)
                explore = (
                    c.get("ft_plan", 0)
                    + c.get("etg_relaxed", 0)
                    + c.get("fuzzy_plan", 0)
                ...
                print(
                    ...
                    f"exploit={exploit} "
                    f"(etg_plan={c.get('etg_plan', 0)} etg_follow={c.get('etg_follow', 0)}) | "
                    f"explore={explore} tuning={tuning_count} "
                    f"(ft_plan={c.get('ft_plan', 0)} etg_relaxed={c.get('etg_relaxed', 0)} "
        ...
        if (my_units or enemy_units) and not suppress_replay_tail_record and not suppress_eval_warmup_record:
            ...
            if (
                ...
                if reward_mode == "etg_correct" and self._etg_sam is not None and nid is not None:
                    sam = self._etg_sam
                    if nid in sam and action_code in sam[nid]:
                        try:
                            gamma = self._beam_params.get("etg_gamma", 0.9)
                            etg_stats = sam[nid][action_code]
                            etg_expected = float(
                                etg_stats.avg_step_reward
                            ) + gamma * float(etg_stats.avg_future_reward)
        ...
        if end_flag and not self._prev_end_game_flag:
            if self._ep_history:
                ...
                if self._mode != "replay":
                    exploit_count = c.get("etg_plan", 0) + c.get("etg_follow", 0)
                    explore_count = (
                        c.get("ft_plan", 0)
                        + c.get("etg_relaxed", 0)
                        + c.get("fuzzy_plan", 0)
