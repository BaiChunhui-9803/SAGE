"""
Experience Transition Graph Visualizer Module

This module provides static visualization tools for decision experience transition graphs.
Generates various charts and reports to validate and analyze the experience transition graph.
...
class ExperienceTransitionGraphVisualizer:
    """
    Static visualizer for Decision Experience Transition Graph.
    ...
    Example:
        ETG = DecisionExperienceTransitionGraph.load("cache/experience_transition_graph/etg_simple.pkl")
        viz = ExperienceTransitionGraphVisualizer(ETG)
        viz.plot_action_quality_heatmap(top_states=50, save_path="heatmap.png")
        viz.generate_summary_report(output_dir="output/figures/experience_transition_graph")
    """
    ...
    def __init__(self, ETG: DecisionExperienceTransitionGraph):
        self.ETG = ETG
        self.setup_style()
    ...
    def _extract_all_stats(
        ...
        for key, action_dict in self.ETG.state_action_map.items():
    ...
    def plot_top_k_distribution(
        ...
        """
        state_visits = Counter()
        for key in self.ETG.state_action_map.keys():
            state = key if isinstance(key, int) else key[0]
            for action, stats in self.ETG.state_action_map[key].items():
                state_visits[state] += stats.visits
    ...
    def plot_metrics_summary(
        ...
        stats = self.ETG.get_statistics()
        metrics_text = f"""
Experience Transition Graph Statistics Summary
{"=" * 50}
...
Context Window:            {stats["context_window"] if stats["use_context"] else "N/A"}
        ...
        fig.suptitle(
            "Experience Transition Graph Metrics Dashboard", fontsize=16, fontweight="bold"
        )
    ...
    def generate_summary_report(self, output_dir: Path) -> Path:
        ...
        report_path = output_dir / "etg_summary_report.md"

        with open(report_path, "w", encoding="utf-8") as f:
            f.write("# Experience Transition Graph Summary Report\n\n")
            ...
            f.write("\n---\n")
            f.write("*Generated by PredictionRTS Experience Transition Graph Visualizer*\n")
