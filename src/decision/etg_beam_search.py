"""
ETG Beam Search — Chain Reasoning Prediction Engine

Performs beam search over the experience transition graph to predict multi-step
...
Core idea:
  ...
  (from ETG quality ranking) and their transition probabilities (from
...
Unified planning entry point:
    ...
    plan = plan_action(ETG, transitions, state=42,
                       action_strategy="best_beam")
    print(plan.recommended_action, plan.action_plan)

Backward-compatible shortcut:
    action, info = find_optimal_action(ETG, transitions, state=42)
"""
...
def beam_search_predict(
    ...
    """
    Beam search over the ETG transition graph.

    Args:
        ETG: DecisionExperienceTransitionGraph instance.
    ...
    for step in range(max_steps):
        ...
        for head_idx, beam in beam_heads:
            top_actions = ETG.get_top_k_actions(
                state=beam.state,
            ...
            for action, quality in top_actions:
                ...
                for ns, count in sorted(
                    ...
                    ns_quality = ETG.get_action_quality(beam.state, action)
