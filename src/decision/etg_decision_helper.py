"""
ETGDecisionHelper - Decision Helper with Experience Transition Graph

Provides a clean interface for real-time decision making using the experience transition graph.
...
Usage:
    # Simulation scenario
    helper = ETGDecisionHelper("cache/experience_transition_graph/etg_simple.pkl")
    state = 42
...
class ETGDecisionHelper:
    """
    Experience Transition Graph Decision Helper

    Provides action recommendations and trajectory predictions based on the experience transition graph.
    """

    def __init__(self, etg_path: str, cfg=None):
        """
        Initialize the helper.

        Args:
            etg_path: Path to experience transition graph pickle file
            cfg: Optional config dict (with 'paths' key). If None, uses get_config().
        """
        self.etg_path = etg_path
        self.cfg = cfg
        self.ETG = DecisionExperienceTransitionGraph.load(etg_path)
    ...
    def _load_transitions(self):
        ...
        transitions_path = self.etg_path.replace(".pkl", "_transitions.pkl")
    ...
    def select_action(
        ...
        """
        top_k_actions = self.ETG.get_top_k_actions(
            state=state,
            history=history,
            k=k,
        ...
        if strategy == "roulette":
            ...
            for action, prob in zip(action_list, probs):
                ...
                if r <= cumsum:
                    ...
                    selected_info = self.ETG.get_action_quality(state, action, history)
                    break
            else:
    ...
    def get_top_k_actions(
        ...
        return self.ETG.get_top_k_actions(
    ...
    def predict_trajectory(
        ...
        quality = self.ETG.get_action_quality(current_state, current_action)
        if not quality:
            return []
