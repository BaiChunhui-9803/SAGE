"""
Decision Experience Transition Graph Module

This module implements a decision experience transition graph that stores state-action-reward statistics
...
class DecisionExperienceTransitionGraph:
    """
    Decision Experience Transition Graph
    ...
    Example usage:
        ETG = DecisionExperienceTransitionGraph(use_context=True, context_window=5)
        ETG.build_from_data(states, actions, rewards, outcomes)
        top_actions = ETG.get_top_k_actions(state=42, history=['4d', '3a'], k=5)
    """
    ...
    def __init__(
        ...
        """
        Initialize experience transition graph
    ...
    def add_visit(
        ...
        """
        Add a state-action visit to the experience transition graph
    ...
    def build_from_data(
        ...
        """
        Build experience transition graph from episode data
        ...
        if verbose:
            logger.info(
                f"Building experience transition graph from {len(state_episodes)} episodes..."
            )
        ...
        if verbose:
            logger.info(f"Experience Transition Graph built:")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        """Get overall statistics about the experience transition graph"""
    ...
    def save(self, path: str):
        """Save experience transition graph to file"""
        data = {
            "use_context": self.use_context,
        ...
        logger.info(f"Experience Transition Graph saved to {path}")
    ...
    def load(cls, path: str) -> "DecisionExperienceTransitionGraph":
        ...
        ETG.state_action_map = defaultdict(dict, data["state_action_map"])
        ETG.total_visits = data["total_visits"]
        ETG.total_trajectories = data["total_trajectories"]
        ETG.unique_states = data["unique_states"]
        ETG.unique_actions = data["unique_actions"]

        logger.info(f"Experience Transition Graph loaded from {path}")
        return ETG
