...
from etg_web.i18n import language, render_language_switcher, st, translate
...
def main(language_code=None):
    st.set_page_config(
        page_title="SAGE Artifact Explorer",
        page_icon="🕸️",
        layout="wide",
    )
    ...
    with st.sidebar:
        if language_code is None:
            ...
            st.session_state["ui_language"] = language_code
        advanced = st.toggle("显示高级实验功能", value=False, key="advanced_mode")
        visible_tabs = [_TAB_OPTIONS[i] for i in ([10, 0, 1, 2, 3, 4, 5, 6, 7] if advanced else [10, 0, 1, 2, 3])]
        # Batch orchestration/data deletion remain in source, outside the review UI.
        _sel = st.segmented_control(
            "功能",
            visible_tabs,
            default=visible_tabs[0],
            key="tab_selector",
        )
        if _sel:
            _sel_val = _sel[0] if isinstance(_sel, list) else _sel
            st.session_state._tab_value = _sel_val
        _sel_val = st.session_state.get("_tab_value", visible_tabs[0])
        if _sel_val not in visible_tabs:
            _sel_val = visible_tabs[0]
        ...
        if active_tab in {5, 7}:
            from importlib.util import find_spec
            required = ["fastapi", "sse_starlette", "pysc2"] if active_tab == 5 else ["optuna"]
            missing = [name for name in required if find_spec(name) is None]
            if missing:
                st.info(("此功能需要单独的 SC2 实验环境，详见 ARTIFACT.md。缺少：" if language() == "zh" else
                         "This feature needs the separate SC2 environment; see ARTIFACT.md. Missing: ") + ", ".join(missing))
                st.stop()
        ...
        if requires_etg_entry:
            ...
            if not advanced:
                catalog = [entry for entry in catalog if entry.get("type") == "augmented"]
            if not catalog:
            ...
            selected_map = st.selectbox("地图", map_ids, key="map_selector")
            ...
            if requires_etg_data:
                ...
                if st.session_state.get("quality_graph") != etg_entry["file"]:
                    st.session_state.quality_low = quality_min
                    st.session_state.quality_high = quality_max
                    st.session_state.quality_graph = etg_entry["file"]
                    st.session_state.pop("viz_quality", None)
        ...
        if requires_etg_entry and not tab_just_switched:
            if active_tab == 1:
                ...
                from etg_web.live_game_tab import _render_live_game_sidebar
                _render_live_game_sidebar(etg_entry)

            elif active_tab == 7:
                from etg_web.learner_tab import _render_learner_sidebar
                _render_learner_sidebar(etg_entry)

    st.title("SAGE · 论文与经验图")
    st.markdown("交互式浏览经验转移图 + 图上束搜索规划。")
    st.markdown(f"### {_TAB_ICONS[active_tab]} {translate(_TAB_OPTIONS[active_tab])}")
    ...
    if active_tab == 0:
        readme_path = Path(__file__).parent.parent / ("README.zh-CN.md" if language() == "zh" else "README.md")
        ...
        from etg_web.live_game_tab import _render_live_game_content
        ...
        from etg_web.learner_tab import _render_learner_tab
        _render_learner_tab()
