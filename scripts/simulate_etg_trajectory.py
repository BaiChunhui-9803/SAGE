...
using the experience transition graph for action recommendations and predicting next states to validate
whether the trajectory can reach a winning terminal state.

Usage:
    python scripts/simulate_etg_trajectory.py --episodes 100
    python scripts/simulate_etg_trajectory.py --mode network --episodes 100
    python scripts/simulate_etg_trajectory.py --verbose --save-trajectories output/trajectories.json
...
class TrajectorySimulator:
    ...
    Simulates decision trajectories from an initial state using experience transition graph
...
def main():
    parser = argparse.ArgumentParser(
        description="Simulate decision trajectories using experience transition graph"
    )
    parser.add_argument(
    ...
    if args.etg_type == "simple":
        ...
        transitions_path = etg_dir / "etg_simple_transitions.pkl"
    else:
        etg_path = etg_dir / f"etg_context_{args.context_window}.pkl"
        transitions_path = etg_dir / f"etg_context_{args.context_window}_transitions.pkl"
    ...
    logger.info(f"  States: {len(ETG.unique_states)}, Actions: {len(ETG.unique_actions)}")
