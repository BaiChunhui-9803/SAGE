...
.fp-badge-etg_plan{background:rgba(13,71,161,.25);color:#64b5f6}
.fp-badge-etg_follow{background:rgba(27,94,32,.2);color:#81c784}
...
<div class="card">
  ...
  <div class="metrics">
    ...
    <div class="metric"><div class="metric-label">ETG</div><div class="metric-value" id="etg-status" style="font-size:11px">--</div></div>
...
function showFramePopup(ep,frIdx,el){
  ...
  var etLbl={etg_plan:'规划',etg_follow:'跟随',diverge:'偏离',backup_switch_exact:'备选(精准)',backup_switch_fuzzy:'备选(模糊)',backup_switch:'备选',fallback:'回退',replay:'回放',external:'外部',no_action:'无动作',manual:'手动'}[et]||et;
  ...
  for(var i=0;i<eps.length;i++){
    ...
    if(ep.markov_flow&&ep.markov_flow.length>0){
      ...
      for(var j=0;j<maxShow;j++){
        ...
        var lbl={etg_plan:'规划',etg_follow:'跟随',diverge:'偏离',backup_switch_exact:'备选(精准)',backup_switch_fuzzy:'备选(模糊)',backup_switch:'备选',fallback:'回退',replay:'回放',external:'外部',no_action:'-',manual:'手动'}[et]||et;
...
function _etDotColor(et){
  var m={etg_plan:'#42a5f5',etg_follow:'#81c784',diverge:'#ffb74d',fallback:'#ef9a9a',backup_switch:'#ce93d8',backup_switch_exact:'#ce93d8',backup_switch_fuzzy:'#b39ddb',no_action:'#888',manual:'#90caf9'};
  return m[et]||'#888';
}
function _forkStateColor(et){
  var m={etg_plan:'#1565c0',etg_follow:'#2e7d32',diverge:'#e65100',fallback:'#b71c1c',backup_switch:'#7b1fa2',backup_switch_exact:'#7b1fa2',backup_switch_fuzzy:'#9575cd',no_action:'#616161',manual:'#1565c0'};
  return m[et]||'#616161';
}
