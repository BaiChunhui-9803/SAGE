...
from etg_web.constants import etg_DIR
...
@st.fragment
def _run_prediction(etg_data, transitions, etg_entry, ETG):
...
def _render_prediction_tab(etg_data, transitions, etg_entry, ETG):
    st.markdown("### 从指定起始状态出发，用 Beam Search 在转移图上进行多步规划搜索。")
    _run_prediction(etg_data, transitions, etg_entry, ETG)
