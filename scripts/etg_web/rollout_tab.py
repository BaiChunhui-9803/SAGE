...
from etg_web.constants import etg_DIR, NPY_DIR
from etg_web.loaders import load_episode_data, load_distance_matrix_np
from etg_web.beam_utils import (
    _compute_composite_scores,
    _build_rec_rows,
    _build_path_detail_rows,
    _results_to_json,
)
from etg_web.pyvis_renderer import render_beam_tree
...
def _cached_rollout_results(
    ...
    ETG = DecisionExperienceTransitionGraph.load(str(etg_DIR / etg_path))
    with open(str(etg_DIR / trans_path), "rb") as f:
        transitions = load_compatible_pickle(f)

    dist_matrix = None
    if enable_backup:
        from etg_web.loaders import load_etg_catalog
        entry = next(e for e in load_etg_catalog() if e["file"] == etg_path)
        dist_matrix = load_distance_matrix_np(entry["map_id"], entry["data_id"])
        if dist_matrix is None:
            raise FileNotFoundError("Backup switching requires the graph's archived distance assets")
...
@st.fragment
def _run_rollout(etg_data, transitions, etg_entry, ETG):
    ...
    unique_states = etg_data.get("unique_states", set())
    ...
    etg_file = etg_entry.get("file", "")
    trans_file = etg_entry.get("transitions", "")
    ...
    with col_mid:
        ...
        st.markdown("**🔍 片段匹配**")
        map_id = etg_entry.get("map_id", "")
        data_id = etg_entry.get("data_id", "")
        match_rows = []
        dist_mat = None
...
def _render_rollout_tab(etg_data, transitions, etg_entry, ETG):
    st.markdown("### 从指定起始状态出发，按策略逐步滚动推演至终端状态。")
    _run_rollout(etg_data, transitions, etg_entry, ETG)
