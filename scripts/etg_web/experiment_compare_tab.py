...
from etg_web.teacher_guided_tab import render_teacher_guided_etg_panel
...
def _load_etg_catalog_entries() -> List[Dict[str, Any]]:
    if not _etg_CATALOG_PATH.exists():
        return []
    try:
        with open(str(_etg_CATALOG_PATH), "r", encoding="utf-8") as f:
        ...
        experiments.append(
            {
                ...
                "etg_name": manifest.get("etg_name", "-"),
...
def _find_catalog_entry(catalog: List[Dict[str, Any]], scenario: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    preferred = scenario.get("preferred_etg")
...
def _scene_completion_rows(experiments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    catalog = _load_etg_catalog_entries()
    rows = []
    for scenario in _SCENARIOS:
        ...
        etg_file_ok = bool(catalog_ok and entry and entry.get("file"))
...
def _render_manifest_template(selected: Dict[str, Any]) -> None:
    ...
    template = {
        ...
        "etg_name": "MvsM4 - Augmented",
        "etg_file": "MarineMicro_MvsM_4_augmented/etg_simple.pkl",
        "transitions": "MarineMicro_MvsM_4_augmented/etg_simple_transitions.pkl",
...
def _build_multistep_parameter_search_command(
    ...
    if scenarios:
        cmd.extend(["--etg-type", str(etg_type)])
            ...
            for line in f:
                ...
                for idx, frame in enumerate(frames):
                    ...
                    if source:
                        ...
                        if source not in {"etg_plan", "etg_follow"}:
                            non_primary_count += 1
...
def _build_replay_baseline_cmd_list(
    ...
    cmd = [
        ...
        "--etg-name",
        str(entry.get("name") or scenario.get("preferred_etg", "")),
        "--etg-file",
...
- `非主ETG动作`：动作来源不是 `etg_plan/etg_follow` 的比例，用来观察是否依赖 relaxed、OOD、fallback、tuning 或 diverge 通道。
...
def _render_multistep_sensitivity_panel(experiments: List[Dict[str, Any]]) -> None:
    ...
    etg_type = r3.selectbox(
        "ETG 数据类型",
        ["augmented", "simple", "any"],
        index=0,
        key="multistep_param_search_etg_type",
        help="默认使用论文主流程的 augmented ETG，不再从旧归档实验文件夹读取参数。",
    )
    ...
    selected_scenarios = st.multiselect(
        ...
        help="按场景从 configs/etg_catalog.yaml 读取 etg/data，新建 training run 并保存到新的 run_XXXX 文件夹。",
    )
    if not selected_scenarios:
        ...
        st.json(
            {
                ...
                "etg_name": selected["etg_name"],
                "etg_file": selected["etg_file"],
