...
def _build_etg_cmd(
    ...
    cmd = [
        ...
        data_id,
        "--output-dir",
        str(Path("cache") / "experience_transition_graph" / f"{map_id}_teacher_{_sanitize(algorithm)}_projected"),
        "--etg-name",
...
def render_teacher_guided_etg_panel() -> None:
    ...
    validate = f3.toggle("构建后 validate ETG", value=False, key="teacher_validate")
