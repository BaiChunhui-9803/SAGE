...
from etg_web.graph_builder import build_graph_data
from etg_web.pyvis_renderer import render_pyvis_html
...
def _render_visualization_tab(
    ...
    n_states = len(etg_data.get("unique_states", set()))
    n_actions = len(etg_data.get("unique_actions", set()))
    total_visits = etg_data.get("total_visits", 0)
    ...
    with st.expander("🔍 状态搜索"):
        ...
        if st.button("搜索", key="search_btn"):
            if search_id in state_stats:
                ...
                key = search_id if not etg_data["use_context"] else (search_id, ())
                actions = etg_data["state_action_map"].get(key, {})
                if actions:
